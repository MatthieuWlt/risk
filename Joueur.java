...
public class Joueur {
	...
	public void attribution_mission(int numero_mission, int choix, Joueur j){
		...
		case 1 :
			Random rand = new Random();
			System.out.println("La mission du joueur"+j.getNumero()+" est de détruire le joueur"+rand.nextInt(choix - 1)+ 1);
			break;
		case 2 :
			System.out.println("La mission du joueur"+j.getNumero()+" est de conquérir tous les territoires");
			break;
		case 3 :
			System.out.println("La mission du joueur"+j.getNumero()+" est de contrôler 3 régions et au moins 18 territoires");
			break;
		case 4 :
			System.out.println("La mission du joueur"+j.getNumero()+" est de contrôler 18 territoires avec au moins 2 armées");
			break;
		case 5 :
		...
		case 7 :
			...
			break;
		case 8 :
			System.out.println("La mission du joueur"+j.getNumero()+" est de contrôler la plus grosse région et une autre région()");
			break;
		}
	}
						...
						while(a!=0) {
							...
							while(a!=0) {
								...
								if (verif(t1,t2)==true) {
									...
									PrintTab(def);
									if((att.length == 3 && def.length==2) ||(att.length==2 && def.length==2)) { //situation possible de combat
										if (att[0]>def[0]&&att[1]<=def[1]||att[0]<=def[0]&&att[1]>def[1]) { //résultat possible aux dés
										System.out.println("le Joueur"+j1.getNumero()+" gagne mais perd 1 unité");
										sup_armee(t2,1); //suppression d'une unité sur le territoire du joueur défenseur
										sup_armee(t1,1); //suppression d'une unité sur le territoire du joueur attaquant 
									...
									else if((att.length == 3 && def.length==1)||(att.length == 2 && def.length==1)||(att.length == 1 && def.length==1)) {
										if(att[0]>def[0]) { 
											sup_armee(t2,1);
											add_armee(t2,(t1.getA()-1)); //déplacement armée joueur attaquant
											sup_armee(t1,(t1.getA()-1)); 
									...
									t1.position(t1,j1.getCouleur()); //renvoie la couleur couleur du joueur lorsqu'il se trompe
								}
								...
								while(d!=0) {
									if (StdDraw.mousePressed()) {//clique sur le territoire de destination
										...
										if (verif(t1,t2)==true) {
											...
											t1.position(t1,j.getCouleur()); //renvoie la couleur couleur du joueur lorsqu'il se trompe
										}
										StdDraw.pause(500);
										break;
									}
