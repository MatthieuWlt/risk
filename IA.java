...
public class IA {
	...
	public void IA_phase_attaque(int a, Joueur j1, Joueur j2, Joueur j3, Joueur j4, Joueur j5, Joueur j6)
	...
	public void IA_phase_attaque2(territoire t1, territoire t2, Joueur j1, Joueur j2) throws InterruptedException {
		...
		if ((att.length == 3 && def.length == 2) || (att.length == 2 && def.length == 2)) { // situation possible de
																							// combat
			if (att[0] > def[0] && att[1] <= def[1] || att[0] <= def[0] && att[1] > def[1]) { // résultat possible aux
																								// dés
				StdDraw.setPenColor(StdDraw.ORANGE);
				StdDraw.filledRectangle(50, 105, 35, 5);// chat-box
				StdDraw.setPenColor(StdDraw.BLACK);
		...
		} else if ((att.length == 3 && def.length == 1) || (att.length == 2 && def.length == 1)
